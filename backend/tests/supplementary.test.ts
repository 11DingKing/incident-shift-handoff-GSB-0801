...
describe('Supplementary handoff packages', () => {
  ...
  async function setupAcknowledgedParent(): Promise<string> {
    await app.inject({
      ...
      payload: { handoff_id: 'hnd-parent', from_shift: 'A', to_shift: 'B', summary: 'initial' },
    });
    await app.inject({
      method: 'POST',
  ...
  it('snapshots only new/changed items with per-field diffs; new timeline event included', async () => {
    ...
    const items = (await app.inject({ method: 'GET', url: `/api/incidents/${INCIDENT}/action-items` })).json();
    ...
    expect(body.items.find((i: any) => i.action_item_id === AI_TEMP)).toBeUndefined();
    ...
    const ai03Added = diffs.find((d: any) => d.ref_id === ai03 && d.change_kind === 'added');
    expect(ai03Added).toBeTruthy();
    const statusDiff = diffs.find((d: any) => d.ref_id === AI_ROUTE && d.field === 'status');
  ...
  it('does not modify the parent package owner/status/version/acknowledgments', async () => {
    const parentId = await setupAcknowledgedParent();
    const before = (await app.inject({ method: 'GET', url: `/api/handoffs/${parentId}` })).json();
    ...
    await app.inject({
      ...
      url: `/api/incidents/${INCIDENT}/handoffs/supplementary`,
      headers: { 'content-type': 'application/json' },
      payload: { handoff_id: 'hnd-supp-2', parent_handoff_id: parentId, from_shift: 'X', to_shift: 'Y', summary: '' },
    });

    const after = (await app.inject({ method: 'GET', url: `/api/handoffs/${parentId}` })).json();
  ...
  it('rejects supplementary creation before parent is acknowledged (409)', async () => {
    await app.inject({
      ...
      url: `/api/incidents/${INCIDENT}/handoffs`,
      headers: { 'content-type': 'application/json' },
      payload: { handoff_id: 'hnd-unacked', from_shift: 'A', to_shift: 'B', summary: '' },
    });
    const res = await app.inject({
      method: 'POST',
      url: `/api/incidents/${INCIDENT}/handoffs/supplementary`,
      headers: { 'content-type': 'application/json' },
      payload: { handoff_id: 'hnd-supp-x', parent_handoff_id: 'hnd-unacked', from_shift: 'X', to_shift: 'Y', summary: '' },
    });
  ...
  it('is idempotent: same idempotency key returns same package, no extra diffs/audit', async () => {
    ...
    const children = (await app.inject({ method: 'GET', url: `/api/incidents/${INCIDENT}/handoffs` })).json()
      .filter((h: any) => h.parent_handoff_id === parentId);
    expect(children).toHaveLength(1);

    const detail = (await app.inject({ method: 'GET', url: '/api/handoffs/hnd-supp-idem' })).json();
  ...
  it('concurrent calls with same idempotency key produce exactly one package and one diff set', async () => {
    ...
    const ids = new Set([c1.json().handoff.handoff_id, c2.json().handoff.handoff_id]);
