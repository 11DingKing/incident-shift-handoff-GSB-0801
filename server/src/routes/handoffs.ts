...
export function handoffRoutes(app: FastifyInstance, pool: Pool): void {
  ...
  app.post('/api/incidents/:incidentId/handoffs', async (req, reply) => {
    ...
    const { rows } = await pool.query(
      `INSERT INTO handoffs (id, incident_id, from_shift, to_shift, note, created_by)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [id, incidentId, fromShift, toShift, body.note ?? '', createdBy],
    );
  ...
  app.get('/api/handoffs/:handoffId', async (req) => {
    ...
    return { handoff, items, supplements };
  });
    ...
    return withTx(pool, async (c) => {
      ...
      await c.query(
        `INSERT INTO timeline_events
         ...
         VALUES ($1, $2, $3, 'audit', '交接包已签收', $4, $5, now())`,
        [
          newId('au'),
          handoff.incident_id,
          handoffId,
          `${signedBy} 签收交接包（${handoff.from_shift} → ${handoff.to_shift}），快照 ${snap.rowCount} 项行动项`,
          signedBy,
        ],
      );
      return { handoff: signed.rows[0], snapshotCount: snap.rowCount };
    });
  });
